  ...
  <v-container fluid>
        ...
        <v-expansion-panel class="elevation-0">
          ...
          <v-expansion-panel-content>
            <v-row>
              ...
              <v-col cols="12" md="3">
                <v-text-field color="#3A1C71" label="Date règlement début" class="pt-5 pa-2"
                  prepend-inner-icon="mdi-magnify" v-model="receptionServiceSendDateStart" type="date" filled dense outlined
                  flat background-color="grey lighten-4" rounded></v-text-field>
              </v-col>
              <v-col cols="12" md="3">
                <v-text-field color="#3A1C71" label="Date règlement fin" class="pt-5 pa-2"
    ...
    <template>
      ...
      <v-row justify="center">
          ...
          <v-card style="border-left: 10px solid #3A1C71">
            ...
            <v-card-text>
              <v-row>
                ...
                <v-col cols="12" sm="6" md="4">
                  <v-select :items="payementItem" filled label="Modes de paiement" color="#3A1C71"
                    v-model="morePayementStudy.paymentMethod"></v-select>
                </v-col>
                <v-col cols="12" sm="6" md="4">
                  <v-text-field label="Date offre" type="date" v-model="morePayementStudy.offerDate" color="#3A1C71"
                    filled></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="4">
                  <v-text-field label="Date accord" type="date" v-model="morePayementStudy.agreementDate"
          ...
          <v-card style="border-left: 10px solid #3A1C71">
            ...
            <v-card-text>
              <v-row>
                ...
                <v-col cols="12" sm="6" md="3">
                  <v-select :items="payementItem" filled label="Modes de paiement" color="#3A1C71"
                    v-model="indemnityStudy.paymentMethod"></v-select>
                </v-col>
                <v-col cols="12" sm="6" md="4">
                ...
                <v-col cols="12" sm="6" md="4">
                  <v-text-field label="Date accord" type="date" v-model="indemnityStudy.agreementDate"
                    color="#3A1C71" filled></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="4">
                  <v-text-field label="Date deuxieme signature" background-color="#ffb703"
        ...
        <v-dialog v-model="addFileMoreNeneficiary" max-width="1000px">
          <v-card style="border-left: 10px solid #3A1C71">
            <v-card-title>
            ...
            <v-card-text>
              <v-row>
                ...
                <v-col cols="12" sm="6" md="4">
                  <v-select :items="payementItem" filled label="Modes de paiement" color="#3A1C71"
                    v-model="beneficiary.paymentMethod"></v-select>
                </v-col>
                <v-col cols="12" sm="6" md="4">
        ...
        <v-dialog v-model="addProvisioneneficiary" max-width="1000px">
          <v-card style="border-left: 10px solid #3A1C71">
            <v-card-title>
            ...
            <v-card-text>
              <v-row>
                ...
                <v-col cols="12" sm="6" md="3">
                  <v-select :items="payementItem" filled label="Modes de paiement" color="#3A1C71"
                    v-model="beneficiaryProvisionData.paymentMethod"></v-select>
                </v-col>
                <v-col cols="12" sm="6" md="3">
                  <v-text-field label="Date offre" type="date" v-model="beneficiaryProvisionData.offerDate" color="#3A1C71"
                    filled></v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="3">
      ...
      <div>
        <v-data-table :headers="headers" :items="offer" :items-per-page="10" local="fr" class="elevation-0"
          ...
          <template v-slot:top>
            <v-toolbar flat>
              <!-- @click="addNewVictime()" -->
              <v-btn class="btnDesign" @click="pvManageFunction()" disabled="true" color="#3A1C71" text-color="white"
                pill>
                Règlement PV
                <v-icon class="ml-2" left> mdi-plus-box-multiple </v-icon>
              </v-btn>
              <v-spacer></v-spacer>
              <v-btn class="mr-2 btnDesign" :loading="loading" :disabled="loading" color="#3A1C71" text-color="white" pill
                @click="addNewVictime()">
                Bordereau de demande de chÈques
                <v-icon class="ml-2" left> mdi-plus-box-multiple </v-icon>
              </v-btn>
            </v-toolbar>
          </template>
...
//lert(this.ayantdroitSelectID)
    ...
    async addIndemnityFormModal() {
      ...
      try {
        const schema = {
          ...
          agreementDate: this.indemnityStudy.agreementDate,
          paymentMethod: this.indemnityStudy.paymentMethod,
      ...
      try {
        const schema = {
          ...
          secondSignatureDate: this.morePayementStudy.secondSignatureDate,
           agreementDate: this.morePayementStudy.agreementDate,
          paymentMethod: this.morePayementStudy.paymentMethod,
    ...
    async postOfBodyStudy() {
      ...
      try {
        const schema = {
          studyOffer: this.bigStudyID,
          consolidationDate: this.bodyStudyOfferData.consolidationDate,
          monthlySMIG: this.bodyStudyOfferData.monthlySMIG,
...
bodyStudyClick(item) {
  this.loading = true;
  this.bigStudyID = item?._id || null;
  this.typeOfStudy = "BodyStudy";
  this.victimeSelectID = item?.sinisterVictim?._id || null; // Correction: _id au lieu de _ids
  ...
  this.getBodyStudyOffer({ id: item._id }).then((res) => {
    if (res && res.length > 0) {
      ...
      this.bodyStudyOfferData.sinisterDate = data?.sinister?.sinisterDate
        ? new Date(data.sinister.sinisterDate).toISOString().split("T")[0]
        : "";

      // Données victime
      this.bodyStudyOfferData.name = data?.sinisterVictim?.fullName || "";
      this.bodyStudyOfferData.age = data?.sinisterVictim?.age || "";

      // Autres données
      this.bodyStudyOfferData.sex = data?.sex || "";
      this.bodyStudyOfferData.painPretiumRate = data?.painPretiumRate?.toString() || "";
      this.bodyStudyOfferData.estheticDamageRate = data?.estheticDamageRate?.toString() || "";
      this.bodyStudyOfferData.careerDamageApplicate = data?.careerDamageApplicate || "";
      this.bodyStudyOfferData.IPRate = data?.IPRate || "";
      this.bodyStudyOfferData.retirementAge = data?.retirementAge?.toString() || "";

      // Dates
      this.bodyStudyOfferData.consolidationDate = data?.consolidationDate
        ? new Date(data.consolidationDate).toISOString().split("T")[0]
        : "";

      // Revenus
      this.bodyStudyOfferData.monthlySMIG = data?.monthlySMIG || 0;
      this.bodyStudyOfferData.monthlySMIGResidence = data?.monthlySMIGResidence || 0;
      this.bodyStudyOfferData.withholdingSMIG = data?.withholdingSMIG || 0;
      this.bodyStudyOfferData.annualSMIG = data?.annualSMIG || 0;
      this.bodyStudyOfferData.monthlyIncome = data?.monthlyIncome || 0;
      this.bodyStudyOfferData.annualIncome = data?.annualIncome || 0;
      this.bodyStudyOfferData.incomeAfterAccident = data?.incomeAfterAccident || 0;

      // ITT
      this.bodyStudyOfferData.applyItt = data?.applyItt || "";
      this.bodyStudyOfferData.numberOfITTDays = data?.numberOfITTDays || 0;
      this.bodyStudyOfferData.ITT = data?.ITT || 0;

      // Dommages
      this.bodyStudyOfferData.fmp = data?.FMP || 0;
      this.bodyStudyOfferData.provision = data?.provision || 0;
      this.bodyStudyOfferData.physiologicalHarm = data?.physiologicalHarm || 0;
      this.bodyStudyOfferData.economicDamage = data?.economicDamage || 0;
      this.bodyStudyOfferData.prejudiceMoral = data?.prejudiceMoral || 0;
      this.bodyStudyOfferData.thirdPartyAssistance = data?.thirdPartyAssistance || 0;
      this.bodyStudyOfferData.pretiumCruciatus = data?.pretiumCruciatus || 0;
      this.bodyStudyOfferData.damageAeshetic = data?.damageAeshetic || 0;
      this.bodyStudyOfferData.careerDamage = data?.careerDamage || 0;
      this.bodyStudyOfferData.bodilyHarm = data?.bodilyHarm || 0;
      this.bodyStudyOfferData.fixedMaterialDamage = data?.fixedMaterialDamage || 0;
      this.bodyStudyOfferData.prosthesisPurchase = data?.prosthesisPurchase || 0;
      this.bodyStudyOfferData.prosthesisMaintenance = data?.prosthesisMaintenance || 0;
      this.bodyStudyOfferData.totalDamage = data?.totalDamage || 0;
      this.bodyStudyOfferData.sharingRate = data?.sharingRate || 0;
      this.bodyStudyOfferData.thirdPartyPayers = data?.thirdPartyPayers || 0;
      this.bodyStudyOfferData.expertFees = data?.expertFees || 0;
      this.bodyStudyOfferData.compensationToBeReceived = data?.compensationToBeReceived || 0;

      // Ajout de code pour déboguer les données qui posent problème
      console.log("Données sinistre:", {
        sinisterNumber: data?.sinister?.sinisterNumber,
        sinisterDate: data?.sinister?.sinisterDate,
        fullName: data?.sinisterVictim?.fullName
      });

      // Si les données ne sont pas correctement extraites, essayons une autre approche
      if (!this.bodyStudyOfferData.sinisterNumber || !this.bodyStudyOfferData.sinisterDate || !this.bodyStudyOfferData.name) {
        console.log("Tentative de récupération des données depuis l'item:", {
          sinisterNumber: item?.sinister?.sinisterNumber,
          sinisterDate: item?.sinister?.sinisterDate,
          fullName: item?.sinisterVictim?.fullName
        });

        // Utiliser les données de l'item si les données de la réponse ne sont pas disponibles
        this.bodyStudyOfferData.sinisterNumber = this.bodyStudyOfferData.sinisterNumber || item?.sinister?.sinisterNumber || "";
        this.bodyStudyOfferData.sinisterDate = this.bodyStudyOfferData.sinisterDate || (item?.sinister?.sinisterDate
          ? new Date(item.sinister.sinisterDate).toISOString().split("T")[0]
          : "");
        this.bodyStudyOfferData.name = this.bodyStudyOfferData.name || item?.sinisterVictim?.fullName || "";
      }
      ...
      console.log("bodyStudyOfferData final:", this.bodyStudyOfferData);
    } else {
      console.warn("⚠️ Aucun résultat dans getBodyStudyOffer.");
      
      // Si aucun résultat, on peut quand même essayer de récupérer les données de base depuis l'item
      this.bodyStudyOfferData.sinisterNumber = item?.sinister?.sinisterNumber || "";
      this.bodyStudyOfferData.sinisterDate = item?.sinister?.sinisterDate
        ? new Date(item.sinister.sinisterDate).toISOString().split("T")[0]
        : "";
      this.bodyStudyOfferData.name = item?.sinisterVictim?.fullName || "";
      
      console.log("Données récupérées depuis l'item:", {
        sinisterNumber: this.bodyStudyOfferData.sinisterNumber,
        sinisterDate: this.bodyStudyOfferData.sinisterDate,
        fullName: this.bodyStudyOfferData.name
      });
    ...
    // En cas d'erreur, on peut quand même essayer de récupérer les données de base depuis l'item
    this.bodyStudyOfferData.sinisterNumber = item?.sinister?.sinisterNumber || "";
    this.bodyStudyOfferData.sinisterDate = item?.sinister?.sinisterDate
      ? new Date(item.sinister.sinisterDate).toISOString().split("T")[0]
      : "";
    ...
    console.log("Données récupérées en cas d'erreur:", {
      sinisterNumber: this.bodyStudyOfferData.sinisterNumber,
      sinisterDate: this.bodyStudyOfferData.sinisterDate,
      fullName: this.bodyStudyOfferData.name
    });
...
this.deathStudyOfferData.annualIncome =
    ...
    handleClickMorePayement(item) {
      ...
      this.morePayementStudy.beneficiaryName = item.fullName;
      this.morePayementStudy.beneficiaryContact = item.contact;
      this.morePayementStudy.nature = item.nature;
      this.morePayementStudy.amount = item.amount;
      this.morePayementStudy.paymentMethod = item.paymentMethod;
      this.morePayementStudy.offerDate = item.offerDate;
      this.morePayementStudy.agreementDate = item.agreementDate;
      ...
      this.morePayementStudy.financialServiceWithdrawalDate = item.financialServiceWithdrawalDate;
      this.morePayementStudy.receptionServiceSendDate = item.receptionServiceSendDate;
    ...
    async addBeneficiaryProvisionFormModal() {
      ...
      try {
        const schema = {
          ...
          offerDate: this.beneficiaryProvisionData.offerDate,
          paymentMethod: this.beneficiaryProvisionData.paymentMethod,
    ...
    async addBeneficiaryFormModal() {
      ...
      try {
        const schema = {
          ...
          paymentMethod: this.beneficiary.paymentMethod,
