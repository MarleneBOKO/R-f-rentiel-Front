...
function locationInfo() {
  ...
  this.confCity = function(id) {
    var url =
      ...
      jQuery("#idPays option:selected").attr("countryId") +
      "&stateId=" +
      jQuery("#idEtat option:selected").attr("stateid") +
      "&cityId=" +
      id;
    var method = "post";
    var data = {};
  ...
  this.getStates = function(id) {
    ...
    call.send(data, url, method, function(data) {
      jQuery(".etat")
        .find("option:eq(0)")
        .html("Sélectionner l'état ou la province");
      if (data.tp == 1) {
        jQuery.each(data["result"], function(key, val) {
          var option = jQuery("<option>");
          option.attr("value", val).text(val);
