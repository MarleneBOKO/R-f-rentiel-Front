<template>
  <div>
    <v-card style="border-top: 10px solid #061a40">
      <v-card-text>
        <h2 class="mt-2 mb-5">
          Adresser une demande à la Direction Technique
        </h2>
        ...
        <v-row>
          ...
          >
            <v-select
              :items="proposedProductNameList"
              v-model="quotingModal.product"
              label="Nom du produit"
            ></v-select>
          </v-col>
          <v-col cols="12" md="12">
            <v-combobox
              v-model="quotingModal.usersEmail"
              :items="userNameList"
              label="Notification Cotation"
              multiple
              chips
            >
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="error" @click="Cotationclose" text>
          Fermer
        </v-btn>
        <v-btn color="success" :loading="loading" @click="saveQuotingRequest">
  ...
  methods: {
    ...
    async initFile() {
      ...
      try {
        ...
        //this.filesListe = this.rdvRequest.files;
      } catch (error) {
    ...
    async saveCall() {
      ...
      try {
        ...
        if (this.callModal._id) {
          ...
          // add
          // console.log(schema);
          await addCall({ schema, id: this.rdvRequest._id });
          defaultMethods.dispatchSuccess(
            this.$store,
    ...
    async makeQuotingRequest(id) {
      try {
        await createQuotingRequest({
          schema: { file: this.rdvRequest._id, product: id },
        });
        defaultMethods.dispatchSuccess(
          this.$store,
    ...
    async saveQuotingRequest() {
      ...
      try {
        ...
        if (this.quotingModal.product) {
          formData.append(
            "product",
            defaultMethods.getItemId(
              this.File.proposedProducts,
              this.quotingModal.product
            )
          );
        }
        // alert(JSON.stringify(this.quotingModal.usersEmail));
        if (this.quotingModal.usersEmail) {
          formData.append(
            "emails",
            JSON.stringify(
              defaultMethods.getItemsEmail(
                this.Users.docs,
                this.quotingModal.usersEmail
              )
            )
          );
        }
        ...
        if (this.rdvRequest._id) {
          formData.append("file", this.rdvRequest._id);
        }
        // add
        await createQuotingRequest({ schema: formData });
        defaultMethods.dispatchSuccess(
          this.$store,
