<template>
  <div>
    <v-card style="border-top: 10px solid #061a40" tile>
      <v-card-text>
            ...
            <v-sheet height="64">
              <v-toolbar flat>
                ...
                <v-btn
                  ...
                  class="ml-5"
                  @click="
                    (callDialog = true), (editedIndex = -1), (callModal = [])
                  "
                >
    ...
    <!-- New appels-->
    <v-dialog v-model="callDialog" max-width="1000px">
      <v-card style="border-top: 10px solid #061a40">
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12">
                <v-select
                  v-model="callModal.assignTo"
                  :items="userNameList"
                  attach
                  chips
                  label="Agent exécutant l'appel"
                  multiple
                ></v-select>
              </v-col>
              <v-col cols="6">
              ...
              <v-col cols="12">
                <quill-editor
                  ref="editor"
                  v-model="callModal.reason"
                  label="Motifs d'appel"
                  class="mt-5"
                  outlined
              ...
              <v-col cols="12">
                <quill-editor
                  ref="editor"
                  v-if="callModal.done"
                  v-model="callModal.returnObservation"
                  label="Retour d'appel"
                  class="mt-5"
                  outlined
                  :options="editorOption"
                />
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn :disabled="loading" color="error" text @click="closeCall">
            Fermer
          </v-btn>
          <v-btn
...
export default {
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1 ? "Gestion Appel" : "Modifier";
    },
  },
  ...
  methods: {
    ...
    async onConfirmQuotationRequestDate() {
      ...
      if (res.success) {
        ...
        await this.getFile(this.callRequest);
      } else {
    ...
    async getinitFile() {
      ...
      try {
        ...
        return new Promise((resolve, reject) => {
          this.getFile(this.callRequest._id)
            .then((res) => {
              resolve(true);
              this.getCallEvent();
              console.log(res);
    ...
    async initFile() {
      this.loading = true;
      try {
        this.File = this.callRequest;
        await this.getProducts({ name: this.searchData });
        await this.getCompanies();
        await this.getUsers();
        await this.getCallEvent();
      } catch (error) {
        defaultMethods.dispatchError(
          this.$store,
          messages.failedToLoad("les dossiers")
    ...
    async saveCall() {
      ...
      try {
        ...
        if (this.callModal._id) {
          ...
          // add
          // console.log(schema);
          await addCall({ schema, id: this.callRequest._id });
          defaultMethods.dispatchSuccess(
            this.$store,
    ...
    async saveQuoting() {
      ...
      try {
        ...
        if (this.quotingModal._id) {
          ...
          // add
          await addQuoting({ schema, id: this.callRequest });
          defaultMethods.dispatchSuccess(
            this.$store,
            messages.createdSuccessfully("Appel")
          );
    ...
    async saveContract() {
      ...
      try {
        ...
        if (this.contractModal._id) {
          ...
          await addContract({ schema, id: this.callRequest });
          defaultMethods.dispatchSuccess(
            this.$store,
