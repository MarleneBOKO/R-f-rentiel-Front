  ...
  <v-container fluid>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title
            >Listing clauses de participation bénéficiaire</v-toolbar-title
          >
...
export default {
  ...
  computed: {
    ...mapGetters(["statPB", "Companies", "Products", "statFilePb"]),
  },
  watch: {
    statCall() {},
  },
  methods: {
    ...
    ...mapActions(["getStatePB", "getCompanies", "getProducts", "statFilePb"]),
    async initStatPB() {
      this.loading = true;
      try {
        await this.getStatePB({
